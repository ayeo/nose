...
impl CopilotConfig {
    fn config_path_inner() -> PathBuf {
        ...
        home.join(".github-copilot").join("hooks.json")
    }
    ...
    fn make_hook_entry(nose_bin: &str, event: &str) -> Value {
        json!({
            "command": format!("{} hook-handler --agent copilot --event {}", nose_bin, event),
            "_nose_managed": true
        })
    }
...
impl AgentConfig for CopilotConfig {
    ...
    fn is_installed(&self) -> bool {
        let home = dirs::home_dir().expect("could not determine home directory");
        home.join(".github-copilot").exists()
    }
    ...
    fn install_hooks(&self, nose_bin: &str) -> Result<String, String> {
        ...
        let config_map = config
            .as_object_mut()
            .ok_or("copilot config is not an object")?;
        ...
        for event in HOOK_EVENTS {
            ...
            arr.retain(|entry| !CopilotConfig::is_nose_managed(entry));
        ...
        Ok(format!(
            "GitHub Copilot: installed hooks for {}",
            installed.join(", ")
        ))
    }
    ...
    fn uninstall_hooks(&self) -> Result<String, String> {
        let path = Self::config_path_inner();
        if !path.exists() {
            return Ok("GitHub Copilot: no config found, nothing to uninstall".to_string());
        }
        ...
        if removed.is_empty() {
            Ok("GitHub Copilot: no nose-managed hooks found".to_string())
        } else {
            Ok(format!(
                "GitHub Copilot: removed hooks for {}",
                removed.join(", ")
            ))
        }
    }
...
mod tests {
    ...
    fn test_make_hook_entry() {
        let entry = CopilotConfig::make_hook_entry("/usr/bin/nose", "preToolUse");
        assert!(entry["command"]
            .as_str()
            .unwrap()
            .contains("hook-handler --agent copilot --event preToolUse"));
    ...
    fn test_is_nose_managed() {
        ...
        assert!(CopilotConfig::is_nose_managed(&managed));

        let not_managed = json!({"command": "something"});
        assert!(!CopilotConfig::is_nose_managed(&not_managed));
    }
}
