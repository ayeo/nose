...
impl CursorConfig {
    fn config_path_inner() -> PathBuf {
        match std::env::consts::OS {
            "macos" => dirs::data_dir()
                .map(|d| d.join("Cursor").join("hooks.json"))
    ...
    fn make_hook_entry(nose_bin: &str, event: &str) -> Value {
        json!({
            "command": format!("{} hook-handler --agent cursor --event {}", nose_bin, event),
            "_nose_managed": true
        })
    }
...
impl AgentConfig for CursorConfig {
    ...
    fn install_hooks(&self, nose_bin: &str) -> Result<String, String> {
        ...
        for event in HOOK_EVENTS {
            ...
            arr.retain(|entry| !CursorConfig::is_nose_managed(entry));
    ...
    fn uninstall_hooks(&self) -> Result<String, String> {
        let path = Self::config_path_inner();
        if !path.exists() {
            return Ok("Cursor: no config found, nothing to uninstall".to_string());
        }
        ...
        if removed.is_empty() {
            Ok("Cursor: no nose-managed hooks found".to_string())
        } else {
            Ok(format!(
                "Cursor: removed hooks for {}",
                removed.join(", ")
            ))
        }
    }
...
mod tests {
    ...
    fn test_make_hook_entry() {
        let entry = CursorConfig::make_hook_entry("/usr/bin/nose", "beforeShellExecution");
        assert!(entry["command"]
            .as_str()
            .unwrap()
            .contains("hook-handler --agent cursor --event beforeShellExecution"));
    ...
    fn test_is_nose_managed() {
        ...
        assert!(CursorConfig::is_nose_managed(&managed));

        let not_managed = json!({"command": "something"});
        assert!(!CursorConfig::is_nose_managed(&not_managed));
    }
}
